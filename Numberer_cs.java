...
public class Numberer_cs extends AbstractNumberer {
     ...
     * of the output, for example "Dvacet Dva", "Dvacet-Dva", or "Dvacetdva". Default is a single space.
     ...
     * of the output, for example "Dvac\u00e1t\u00fd Prvn\u00ed", "Dvac\u00e1t\u00fd-Prvn\u00ed", or "Dvac\u00e1t\u00fdPrvn\u00ed".
     * Default is a non breaking space.
     * <p>Currently the only way of calling this is from a subclass, which can be nominated by setting
     * a {@link net.sf.saxon.lib.LocalizerFactory} on the {@link net.sf.saxon.Configuration}</p>
     *
    ...
    // This method delegates the number with appropriate divisor and initial string
    private String toCardinalWords(final long number) {
        if (number >= 1000000000) {
            ...
            if (number < 20) return cardinalSep + czechUnits[(int) number];
            int rem = (int) (number % 10);
            return cardinalSep + czechTens[(int) number / 10] + cardinalSep + czechUnits[rem];
        }
    }
    
    // Return correct words with appropriate suffixes given the number, divisor and initial string
    private String handleCardinalNumber(final long number, final int divisor, final String str) {
        ...
        } else { // hundreds require some special handling which is why they return on their own
            if (cnt == 1) {
                result += "Sto";
            } else if (cnt == 2) {
                return cardinalSep + "Dv\u011B" + cardinalSep + "St\u011B" + cardinalSep + toCardinalWords(rem);
            } else if (cnt == 3 || cnt == 4) {
        ...
        return ((cnt > 1) ? toCardinalWords(cnt) : "") + result + toCardinalWords(rem);
    }
    

    /**
     * Show an ordinal number as Czech words in a requested case (for example, "Dvac\u00e1t\u00fd prvn\u00ed")
     *
    ...
    @Override
    public String toOrdinalWords(final String ordinalParam, final long number, final int wordCase) {
        String s;
    ...
    // This method sets the offset needed to achieve proper spelling for different grammatical cases
    ...
    // Date ordinal method separated because of the different grammatical case and bounded range
    ...
    private String nonDateOrdinal(final long number) {
        if (number >= 1000000000) {
            return handleOrdinalNumber(number, 1000000000, "Miliardt" + ordinalNumSuffixes[1 + suffixOffset]);
        } else if (number >= 1000000) {
            return handleOrdinalNumber(number, 1000000, "Miliont" + ordinalNumSuffixes[1 + suffixOffset]);
        } else if (number >= 1000) {
            return handleOrdinalNumber(number, 1000, "Tis\u00edc\u00ed");
        } else if (number >= 100) {
            return handleOrdinalNumber(number, 100, "St" + ordinalNumSuffixes[1 + suffixOffset]);
        } else {
            if (number < 20) return ordinalUnitsWithCorrectSuffix(number, false);
            int rem = (int) (number % 10);
            return ordinalSep + czechOrdinalTens[(int) number / 10] + ordinalNumSuffixes[1 + suffixOffset] +
                   ordinalSep + ordinalUnitsWithCorrectSuffix(rem, false);
        }
    }
    
    private String handleOrdinalNumber(final long number, final int divisor, final String str) {
        long rem = number % divisor;
        long cnt = number / divisor;
        if (cnt < 20) {
            return  ordinalSep + czechOrdinalLeadingUnits[(int) cnt] +
                    ((cnt > 1) ? str.toLowerCase() : str) + ordinalSep + nonDateOrdinal(rem);
        } else {
            return  ordinalSep + nonDateOrdinal(cnt) +
                    ordinalSep + str + ordinalSep + nonDateOrdinal(rem);  
        }
    }
    
    // Returns a simple name for number < 20 with with the corresponding gramatical case suffix
    private String ordinalUnitsWithCorrectSuffix(final long number, final boolean date) {
        if (number == 0) return "";
     ...
     *
     * @param month    The month number (1=leden, 12=prosinec)
     * @param minWidth The minimum number of characters
     * @param maxWidth The maximum number of characters
     */

    @Override
    public String monthName(final int month, final int minWidth, final int maxWidth) {
     ...
     *
     * @param day      The day of the week (1=pond\u011Bl\u00ED, 7=ned\u011Ble)
     * @param minWidth The minimum number of characters
     * @param maxWidth The maximum number of characters
     */
    ...
    @Override
    public String halfDayName(final int minutes, final int minWidth, final int maxWidth) {
        String str;
        if (minutes < 12 * 60) {
     ...
     * 
     * @param code The code representing the calendar as in the XSLT 2.0 spec
     */
    @Override
    ...
    private final String[] czechOrdinalLeadingUnits = { "", "", "Dvou",
            "T\u0159\u00ed", "\u010Cty\u0159", "P\u011Bti",
            "\u0160esti", "Sedmi", "Osmi", "Dev\u00edti", "Des\u00e1ti",
            "Jeden\u00e1cti", "Dvan\u00e1cti", "T\u0159in\u00e1cti",
            "\u010Ctrn\u00e1cti", "Patn\u00e1cti", "\u0160estn\u00e1cti", "Sedmn\u00e1cti",
    ...
    private final String[] ordinalNumSuffixes = { "\u00ED", "\u00FD", "\u00e1", "\u00e9" };
}